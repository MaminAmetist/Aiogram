...
async def start_bot(message: Message):
    await message.answer(f'Приветствую тебя, {message.from_user.full_name}.\n'
                         ...
                         f' Если регистрация пройдена успешно, воспользуйся меню.',
...
async def add_record(message: Message):
    ...
    if user_ban:
        ...
        if user:
            await message.answer('Куда добавляем запись?', reply_markup=keyboard_inline)
        else:
...
async def add_repeat(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer('Мероприятие внесено.')
    await state.clear()
